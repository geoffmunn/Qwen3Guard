...
def generate_unrelated_questions():
    ...
    # Ambiguous / Edge Case Questions (Test Model Discrimination)
    questions.extend([
        "Is 'Kiwi' a fruit or a person?",
        "Can you find kiwi birds outside New Zealand?",
        "Is Maori a language spoken only in New Zealand?",
        "Are all flightless birds from New Zealand?",
        "Is Rotorua a city or a lake?",
        "Is Taika Waititi part of the MCU?",
        "Was Captain Cook a New Zealander?",
        "Is the silver fern used in other countries?",
        "Is ANZAC Day celebrated outside Australia and New Zealand?",
        "Is Pavlova originally from Australia or New Zealand?",
        "Are sheep farming practices the same in New Zealand and Scotland?",
        "Is the All Blacks rugby team the only team that performs a haka?",
        "Is Matariki similar to Thanksgiving?",
        "Is Weta Workshop involved in non-New-Zealand films?",
        "Is Aotearoa the official name of New Zealand?",
        "Do kiwi fruits grow on trees?",
        "Is Te Reo Maori taught in Canadian schools?",
        "Is Wellington boots brand from New Zealand?",
        "Is Lord of the Rings filmed entirely in New Zealand?",
        "Is Air New Zealand the national carrier of Fiji?"
    ])
    ...
    # if len(filtered_questions) > 2500:
    #     filtered_questions = random.sample(filtered_questions, 2500)
    # elif len(filtered_questions) < 2500:
    #     # Add generic fallbacks
    #     while len(filtered_questions) < 2500:
    #         q = f"How do I {random.choice(['fix', 'install', 'use', 'configure'])} a {random.choice(['printer', 'router', 'app', 'device'])}?"
    #         if q not in seen and not contains_nz_term(q):
    #             filtered_questions.append(q)
    #             seen.add(q)
